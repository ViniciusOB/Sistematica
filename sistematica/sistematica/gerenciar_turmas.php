...
if (isset($_POST['busca'])) {
    $busca = $_POST['busca'];
    $sql .= " WHERE nome LIKE :busca OR nivel_ensino LIKE :busca";
}
$stmt = $pdo->prepare($sql);
if (isset($busca)) {
    ...
    <div class="content">
        <div class="container mx-auto px-4 py-6 md:py-8">
            <div class="bg-white p-6 md:p-8 rounded-lg shadow-md">
                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-blue-800 mb-4">Gerenciar Turmas</h1>
                ...
                <a href="cadastrar_turmas.php" class="block sm:inline-block bg-blue-800 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded mb-4">Cadastrar Nova Turma</a>
                ...
                <form method="post" action="" class="flex flex-col sm:flex-row sm:items-center mb-6 space-y-4 sm:space-y-0 sm:space-x-4">
                    <input type="text" class="form-control bg-gray-100 border border-gray-300 rounded py-2 px-3 text-gray-700 w-full sm:flex-1 sm:max-w-sm" name="busca" placeholder="Buscar turma">
                ...
                <h2 class="text-lg sm:text-xl md:text-2xl font-bold text-blue-800 mb-4">Lista de Turmas</h2>
                    ...
                    <table class="min-w-full bg-white border border-gray-300">
                        <thead class="hidden sm:table-header-group bg-blue-800 text-white">
                            <tr>
                                ...
                                <th class="p-3 text-left">Nível de Ensino</th>
                            ...
                            <?php foreach ($turmas as $turma): ?>
                                ...
                                <tr class="block sm:table-row border-b">
                                    <td data-label="Nome" class="p-3 sm:table-cell sm:text-left">
                                        <span class="text-gray-900 font-semibold"><?php echo htmlspecialchars($turma['nome']); ?></span>
                                    </td>

                                    <td data-label="Nível de Ensino" class="block p-3 sm:table-cell sm:text-left">
                                        <span class="block sm:hidden font-semibold text-gray-600">Nível de Ensino:</span>
                                        <?php echo htmlspecialchars($turma['nivel_ensino']); ?>
                                    </td>

                                    <td data-label="Ações" class="block p-3 sm:table-cell sm:text-left">
                                        <form method="post" action="">
                                            <input type="hidden" name="excluir" value="<?php echo $turma['id']; ?>">
                                            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded inline-block"
                                                    onclick="return confirm('Tem certeza que deseja excluir esta turma?')">Excluir</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
